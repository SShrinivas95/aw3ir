    ...
    app = new Vue({
        ...
        methods: {
            ...
            remove: function (_city) {
                this.cityList = this.cityList.filter(function (nom) { return nom.name != _city.name })
            },
            meteo: function (_city) {
                this.cityWeatherLoading = true;

                // appel AJAX avec fetch
                fetch('https://api.openweathermap.org/data/2.5/weather?q=' + _city.name + '&units=metric&lang=fr&apikey=34508975d970c7a415e879e401291671')
            ...
            //     for (const nomm of this.cityList) {
            //         if (nomm.name == cityName) {
            //             return true;
            //         }
            //     }
            // },
            isCityExist: function (cityName) {
                return app.cityList.some(function (nom) {
                    return nom.name.toUpperCase() == cityName.toUpperCase();
                })
            },
        }
    });
